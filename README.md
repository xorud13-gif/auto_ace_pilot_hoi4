...
1. **인게임 [결정과 선택(Decisions)] UI 완벽 통합**
   ...
   * **`[에이스 주기적 자동 배치 비활성화 / 활성화]`**: 자동 배치 기능을 언제든 켜고 끌 수 있는 토글 지원 (기본 상태: ON).
...
4. **매일 자정 메인 스레드 정기 점검 (`ACE_AUTO_ASSIGN:DAILY`)**
   * 시간 경과 중 매일 1회 전체 비행단의 빈자리를 안전하게 자동 스캔 및 보충합니다.
5. **100% 메인 게임 스레드 동기화 (크래시 위험 0%)**
   * 클라우제비츠 엔진 내부 로그 디스패처 훅을 통해 모든 연산이 메인 스레드에서 무지연으로 수행되어 게임 멈춤이나 크래시가 발생하지 않습니다.
...
│
├── deploy_to_steam.bat         # 스팀 본편 폴더 원클릭 배포 유틸리티
...
3. 루트 폴더에 새로운 `version.dll`이 자동으로 생성됩니다.
